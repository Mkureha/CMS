...
function setParentText() {
	...
	opener.document.getElementById("busyo_cyu_code").value = document
			...
			if (document.getElementById("Dcodeinput").value == "99") {
				...
				opener.document.getElementById("busyo_name").value = document.getElementById("Dname").value + "/"
			} else {
				if (document.getElementById("Dname").value == document.getElementById("Cname").value) {
					opener.document.getElementById("busyo_name").value = document.getElementById("Dname").value + "/"
				} else {
	opener.document.getElementById("busyo_name").value = document.getElementById("Cname").value + "/"
			    }
				...
				<table class="table table-bordered table-condensed"
					...
					<tr class="form-group">
						<td class="text-center warning" for="busyo_dai_code"
							style="width: 100px; height: auto; text-align: right; font-weight: bold; color: red;">大分類</td>
						<td style="width: 200px;"><select
							class="form-control form-control-sm" name="busyo_dai_code"
							id="Dcodeinput" style="width: 150px; height: auto; left: 10px;">
						...
						<td style="width: 300px"><c:forEach var="lc"
								items="${listcode}">
								<c:if test="${lc.busyo_dai_code eq busyo_dai_code}">
